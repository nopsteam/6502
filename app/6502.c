...
int initSimulator(struct CPU *cpu, struct BUS *bus, int initial_address,
                  char *programPath) {
  srand(time(NULL));
...
void generateRandomByte(struct BUS *bus) { writeBus(0xFE, rand() % 255, bus); }
